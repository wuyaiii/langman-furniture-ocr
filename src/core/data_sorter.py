...
class DataSorter:
    ...
    def sort_excel_data(self):
        ...
        try:
            ...
            # 写入排序结果
            success = self._write_sorted_data(sorted_data, historical_quantities)
    ...
    def _parse_data_groups(self, data_groups):
        ...
        for group in data_groups:
            ...
            logger.debug(f"解析标题: {title} → 年份:{parsed_info['year']}, 类别:{parsed_info['category']}, 套装号:{parsed_info['set_number']}")
    ...
    def _clean_category_name(self, category):
        ...
        modifiers_to_remove = self.category_config.get('modifiers_to_remove', [])
    ...
    def _sort_categories_by_priority(self, categories):
        ...
        # 创建优先级映射
        priority_map = {category: index for index, category in enumerate(priority_order)}
        ...
        unprioritized_categories.sort(key=lambda x: len(categories[x]), reverse=True)
        
        # 合并排序结果
        sorted_category_names = prioritized_categories + unprioritized_categories
        
        logger.info(f"类别排序结果:")
        logger.info(f"  优先级类别 ({len(prioritized_categories)}个): {prioritized_categories}")
        logger.info(f"  其他类别 ({len(unprioritized_categories)}个): {unprioritized_categories}")
    ...
    def _write_sorted_data(self, categories, historical_quantities=None):
        ...
        try:
            workbook = openpyxl.load_workbook(self.excel_manager.excel_file_path)
            ...
            # 使用自定义排序获取类别顺序
            sorted_category_names = self._sort_categories_by_priority(categories)
            ...
            for category in sorted_category_names:
                ...
                logger.debug(f"类别 '{category}' (共{item_count}项) 分配到列 {get_column_letter(current_col)}-{get_column_letter(current_col+3)}")
            ...
            for category in sorted_category_names:
                ...
                for item in items:
                    # 写入套装标题
                    title_cell = sorted_sheet.cell(row=current_row, column=start_col, value=item['original_title'])
                    ...
                    for item_name in item['items']:
                        item_cell = sorted_sheet.cell(row=item_row, column=start_col + 1, value=item_name)
                        ...
                        if item_name in historical_quantities:
                            ...
                            if 'quantity1' in quantities:
                                quantity1_cell = sorted_sheet.cell(row=item_row, column=start_col + 2, value=quantities['quantity1'])
                                quantity1_cell.font = Font(bold=True)
                                quantity1_cell.alignment = Alignment(horizontal='center', vertical='center')
                                logger.debug(f"恢复数量数据: {item_name} -> C列: {quantities['quantity1']}")
                            
                            # 写入第二个数量（D列）
                            if 'quantity2' in quantities:
                                quantity2_cell = sorted_sheet.cell(row=item_row, column=start_col + 3, value=quantities['quantity2'])
                                quantity2_cell.font = Font(bold=True)
                                quantity2_cell.alignment = Alignment(horizontal='center', vertical='center')
                                logger.debug(f"恢复数量数据: {item_name} -> D列: {quantities['quantity2']}")
            ...
            # 应用格式设置
            self._apply_excel_formatting(sorted_sheet, categories, category_positions, current_col)
