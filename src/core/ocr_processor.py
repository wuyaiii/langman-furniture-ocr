...
class OCRProcessor:
    ...
    def extract_title_and_items(self, ocr_result):
        ...
        try:
            # 查找标题
            extracted_title = self._extract_title(ocr_result["TableDetections"])
            
            # 提取物品名
            extracted_items = self._extract_items(ocr_result["TableDetections"])
            
            logger.info(f"提取完成 - 标题: {extracted_title}, 物品数量: {len(extracted_items)}")
        ...
        for table in table_detections:
            if table.get("Type") == 1:  # 数据表格
                ...
                if pic_row is not None:
                    ...
                    if title_candidates:
                        # 选择距离最近的非空文本作为标题
                        closest_text = min(title_candidates, key=lambda x: x['distance'])
    ...
    def _extract_items(self, table_detections):
        ...
        for table in table_detections:
            if table.get("Type") == 1:  # 数据表格
                cells = table.get("Cells", [])
                return self._extract_items_from_table(cells)
